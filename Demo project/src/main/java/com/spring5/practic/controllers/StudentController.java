...
import com.spring5.practic.services.StudentServices;

@Controller
public class StudentController {

	@Autowired
	private StudentServices userService;
//	private Student user;
	...
	public String add(@RequestParam("name") String name, Model model) {
		...
		model.addAttribute("message", "Student creat Successfully");
		return "index";
	}
	@GetMapping("remove-user")
	public String remove(@RequestParam("name") String name, Model model) {
		userService.removeUserByName(name);;
		model.addAttribute("message", "Student Remove Successfully");
		return "index";
	}
